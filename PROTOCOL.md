# Beko AC IR Protocol Documentation

> Work in progress — reverse-engineered from captures on Beko 31225 / 30925.
> Solution is functional; document is being cleaned up.

## Hardware

- **AC unit**: Beko 31225 (protocol likely identical on 30925)
- **IR blaster**: Moes UFO-R11 (Zigbee, model TS1201, Tuya ZS06-equivalent)
- **Integration**: Zigbee2MQTT → Home Assistant
- **Send command**: `zigbee2mqtt/Black Box IR/set` → `{"ir_code_to_send": "<base64>"}`

---

## UFO-R11 Wire Format

```
base64( FastLZ_compress( uint16_le_microseconds[] ) )
```

- Timings alternate: MARK (IR on), SPACE (IR off), starting with leader MARK.
- FastLZ must use proper LZ77 back-references — literal-only encoding produces ~122 bytes
  which exceeds the UFO-R11 Zigbee packet limit (~64–80 bytes) and gets silently truncated.
  Proper compression yields 28–34 bytes.
- Wi-Fi Tuya devices skip FastLZ and send raw uint16 base64.

See [zigbee2mqtt#29701](https://github.com/Koenkk/zigbee2mqtt/issues/29701) and
[zigbee2mqtt#26477](https://github.com/Koenkk/zigbee2mqtt/issues/26477) for the truncation bugs.

---

## Physical Signal

| Parameter     | Value                          |
|---------------|--------------------------------|
| Leader mark   | 2940 µs                        |
| Leader space  | 9700 µs                        |
| Bit mark      | 501 µs                         |
| 0-bit space   | 501 µs                         |
| 1-bit space   | 1559 µs                        |
| Trailing mark | 501 µs                         |
| Total timings | 59 (1 + 1 + 28×2 + 1)         |
| Carrier       | 38 kHz                         |
| Protocol      | PDM, proprietary Arçelik/Beko  |

---

## 28-bit Frame Structure

```
Bit pos:  0    4    8  10   13   16   20   24
          1000 1000 PP FFF  MMM  NNNN FFFF CCCC
```

| Bits  | Field      | Values |
|-------|------------|--------|
| 0–7   | Header     | `10001000` = 0x88, fixed |
| 8–9   | Power      | `00` = ON, `11` = OFF |
| 10–12 | Frame type | `001` = command, `000` = wake/ON, `010` = swing |
| 13–15 | Mode       | 3-bit mode code, see table below |
| 16–19 | Temp       | `frame_temp − 15` |
| 20–23 | Fan        | encoded value, see table below |
| 24–27 | Checksum   | `(fan_val + temp_lo + mode[1:3]) % 16` |

### Mode encoding (bits 13–15)

| Mode     | bits[13:16] | frame_temp   |
|----------|-------------|--------------|
| COOL     | `000`       | actual       |
| HEAT     | `100`       | actual       |
| DEHUM    | `001`       | `16` (fixed) |
| FAN ONLY | `010`       | `16` (fixed) |
| AUTO     | `011`       | actual       |

For DEHUM and FAN ONLY the AC ignores temperature — frame always carries `16`.

Bit 13 acts as a "heat flag": when set, the checksum uses a different `temp_lo` formula
(see Checksum section). Bits 14–15 are the sub-mode selector.

### Fan encoding (bits 20–23)

| Speed | fan_val | Binary |
|-------|---------|--------|
| 1     | 0       | `0000` |
| 2     | 9       | `1001` |
| 3     | 2       | `0010` |
| 4     | 10      | `1010` |
| 5     | 4       | `0100` |

### Checksum (bits 24–27)

Sum of the four nibbles immediately above the checksum (bits 4–27), mod 16:

```python
chk = ((frame >> 4) + (frame >> 8) + (frame >> 12) + (frame >> 16)) & 0xF
```

This is the same as `sumNibbles(state >> 4, 4)` from the LG A/C implementation in IRremoteESP8266.
The formula is **universal**: identical for cmd, wake, swing, and all fixed frames — no
per-frame-type special-casing needed. The apparent differences in the earlier per-type formulas
were just algebraic rearrangements of this single rule.

### OFF frame

OFF is always `0x88C0051` regardless of previous state. bits[10:12] = `000` (not `001`),
so it cannot be generated by the normal formula — it must be hardcoded.

---

## Frame Types

The protocol uses separate frame types — the AC stores state internally; there is no single
"full state" frame.

| bits[10:12] | bits[13:16] | Type     | Description                               |
|-------------|-------------|----------|-------------------------------------------|
| `000`       | mode        | Wake/ON  | Switches AC on + sets mode/temp/fan       |
| `001`       | mode        | Command  | Changes mode/temp/fan (AC must be on)     |
| `010`       | —           | Swing    | Vane position only                        |
| `000`       | `110` (OFF) | OFF      | Fixed frame `0x88C0051`                   |

Checksum is the same universal formula for all types (see Checksum section).

Wake (`000`) and Command (`001`) differ only in bit 12. Because the frame body differs,
the checksum naturally comes out 8 less for wake — that is a consequence of the formula,
not a special rule. The AC ignores Command frames when powered off, which prevents
accidental setting changes.

---

## Swing Frame Structure

Swing frames: bits[10:12] = `010`, bits[13:15] = `11`. Position is encoded in bits[16:24]
as two nibbles `(hi, lo)`. The frame structure differs from command frames and is not
generated by `beko_frame()`.

### Swing positions

| Position | hi | lo | hex        | Description       |
|----------|----|----|------------|-------------------|
| pos1     | 0  | 4  | `0x8813048` | top               |
| pos2     | 0  | 5  | `0x8813059` | —                 |
| pos3     | 0  | 6  | `0x881306A` | —                 |
| pos4     | 0  | 7  | `0x881307B` | —                 |
| pos5     | 0  | 8  | `0x881308C` | —                 |
| pos6     | 0  | 9  | `0x881309D` | bottom            |
| auto     | 1  | 4  | `0x8813149` | continuous sweep  |
| stop     | 1  | 5  | `0x881315A` | hold current position |

Checksum: `(lo + 4 + hi) % 16`

Verified across two independent capture sessions (fan=2 and fan=3, different temperatures) —
swing frame is independent of mode, temperature, and fan speed.

### Yumişak (Soft Air) button

The remote has a dedicated **Yumişak Hava** ("soft air") button that toggles the vane between
pos1 (down) and pos6 (horizontal/up). It is a marketing feature — indirect airflow avoids cold
air blowing directly on people by sending it toward the ceiling first, where it mixes with warm
air before descending. The remote highlights the icon while "soft" mode is active.

In the IR protocol this is not a separate frame type — it is just two alternating swing commands:
`pos1 → pos6 → pos1 → …`. No special handling required.

### Captured base64 (reference)

| Position | hex         | Base64 |
|----------|-------------|--------|
| pos1     | `0x8813048` | `CSoL1yXeASYG3gFAAQEgAoADQA/AC+AHB+ADG0ALQANAI0ABQAfAA0AXQANAAUAHCyAC3gEmBt4B3gHeAQ==` |
| pos2     | `0x8813059` | `CZkL2SXeAScG3gHgAwFADwEiAuANA+ADG0ALQANAAeAHL0AX4AMDC94B3gEnBt4BJwbeAQ==` |
| pos3     | `0x881306A` | `CRkL9CXaASYG2gFAAQMlAtoBQAFAD0AL4AsD4AMbQAtAA+AHJ+AHE0APCyYG2gHaAdoB2gHaAQ==` |
| pos4     | `0x881307B` | `C4gLviXTASIG0wEkAuABA+AHD+APG0AnQAPgAx9AC+ADE0ALwAMHJALTASIG0wE=` |
| pos5     | `0x881308C` | `C2AL3CXgASEG4AEmAuABA+AHD+AHG0ABQBNAJ0ADQAvAAcAPQAfAAcALByYC4AEhBuAB` |
| pos6     | `0x881309D` | `DTwLziXeASAG3gEeAt4BwAHAD0AT4AcD4AMbQAtAA+ADI8AP4AMHQAsL3gHeASAG3gEeAt4B` |
| auto     | `0x8813149` | `C34L6CXcARkG3AEbAuABA+AHD+APG0AnQAPgAx9AC+AHE0APCxkG3AHcAdwBGwLcAQ==` |
| stop     | `0x881315A` | `DTwL1iXmARoG5gEcAuYBQAFAB8APQAvgBwPgAxtAC0AD4AMBQA9AM0AHwAFACwscAuYBHALmARoG5gE=` |

---

## Python Generator (verified on 31+ captures)

```python
FAN_VALS = {1: 0, 2: 9, 3: 2, 4: 10, 5: 4}

# (bits[13], bits[14:16])
MODE_BITS = {
    "cool":     (0, 0b00),
    "heat":     (1, 0b00),
    "dehum":    (0, 0b01),
    "fan_only": (0, 0b10),
    "auto":     (0, 0b11),
}
MODE_FIXED_TEMP = {"dehum": 16, "fan_only": 16}

def _chk(frame: int) -> int:
    s = frame >> 4
    return (s + (s >> 4) + (s >> 8) + (s >> 12)) & 0xF

def beko_frame(mode, temp, fan, wake=False):
    mode_b, mb = MODE_BITS[mode]
    frame_temp = MODE_FIXED_TEMP.get(mode, temp)
    swg = 0b000 if wake else 0b001
    n4  = frame_temp - 15
    fv  = FAN_VALS[fan]
    f   = (0b10001000 << 20 | 0b00 << 18 | swg << 15 | mode_b << 14 |
           mb << 12 | n4 << 8 | fv << 4)
    return f | _chk(f)

OFF_FRAME = 0x88C0051  # fixed, bits[10:12]=000
```

---

## Special Fixed Frames

| Command        | Hex          | bits[10:12] | bits[13:16] | Note |
|----------------|--------------|-------------|-------------|------|
| OFF            | `0x88C0051`  | `000`       | `110`       | fixed |
| Display toggle | `0x88C00A6`  | `000`       | `110`       | backlight toggle, verified on 6 captures |
| Turbo ON       | `0x8810089`  | `010`       | `000`       | fast mode ON |
| Turbo OFF      | —            | —           | —           | = normal send_ir() with current state |
| Sessiz ON      | `0x88C0A6C`  | `000`       | `000`       | outdoor unit silent mode ON |
| Sessiz OFF     | `0x88C0A7D`  | `000`       | `000`       | outdoor unit silent mode OFF |

OFF, Display, and Sessiz share the same frame class: `power=11, type=000`. The remaining
bits are not mode/temp/fan — they carry a fixed command payload. Sessiz reduces noise of
the outdoor unit (compressor + fan); the AC beeps to confirm the command was received.

---

## Verified Generated Hex Values

| Command        | Hex         |
|----------------|-------------|
| OFF            | `0x88C0051` |
| COOL 18° fan1  | `0x880830B` |
| COOL 19° fan1  | `0x880840C` |
| COOL 22° fan1  | `0x880870F` |
| COOL 26° fan1  | `0x8808B03` |
| HEAT 16° fan1  | `0x880C10D` |
| HEAT 20° fan1  | `0x880C501` |
| HEAT 27° fan1  | `0x880CC08` |
| HEAT 27° fan2  | `0x880CC91` |
| HEAT 27° fan3  | `0x880CC2A` |
| HEAT 27° fan4  | `0x880CCA2` |
| HEAT 27° fan5  | `0x880CC4C` |

---

## MQTT Topics

Format: `beko/<device>/set/<command>`

| Topic                          | Payload              | Description         |
|--------------------------------|----------------------|---------------------|
| `beko/<dev>/set/mode`          | `off/cool/heat/dehum/fan_only/auto` | Mode    |
| `beko/<dev>/set/temperature`   | `16`–`30`            | Set temperature     |
| `beko/<dev>/set/temp_up`       | `1`                  | Temperature +1°C    |
| `beko/<dev>/set/temp_down`     | `1`                  | Temperature −1°C    |
| `beko/<dev>/set/fan_mode`      | `1`–`5`              | Fan speed           |
| `beko/<dev>/set/swing`         | `pos1`–`pos6`, `auto` | Vane position      |
| `beko/<dev>/set/display`       | `toggle`             | Backlight toggle    |
| `beko/<dev>/set/turbo`         | `on` / `off`         | Turbo mode          |

---

## Open Questions

| Question | Status |
|----------|--------|
| Swing: position or increment? | Unknown — needs testing |
| Auto fan mode encoding | Unknown |
| Temperature range upper limit | Likely 30°C |
| Does temp/fan matter in swing frame? | Decoded values don't match main formula — may be unused |

---

## Prior Art

**Update**: The Beko/Arçelik 28-bit protocol is identical to the **LG A/C IR protocol** already implemented in IRremoteESP8266
([`ir_LG.h`](https://github.com/crankyoldgit/IRremoteESP8266/blob/master/src/ir_LG.h) /
[`ir_LG.cpp`](https://github.com/crankyoldgit/IRremoteESP8266/blob/master/src/ir_LG.cpp)).
Every constant matches exactly:

| Our name        | LG constant            | Value        |
|-----------------|------------------------|--------------|
| `OFF_FRAME`     | `kLgAcOffCommand`      | `0x88C0051`  |
| `DISPLAY_CODE`  | `kLgAcLightToggle`     | `0x88C00A6`  |
| Fan 1 (0)       | `kLgAcFanLowest`       | `0`          |
| Fan 2 (9)       | `kLgAcFanLowAlt`       | `9`          |
| Fan 3 (2)       | `kLgAcFanMedium`       | `2`          |
| Fan 4 (10)      | `kLgAcFanHigh`         | `10`         |
| Fan 5 (4)       | `kLgAcFanMax`          | `4`          |
| `0x88` header   | `kLgAcSignature`       | `0x88`       |
| Swing 0x8813xxx | `kLgAcSwing*` series   | `0x8813xxx`  |

Arçelik/Beko OEMs the LG protocol without modification. The bits[10:12] frame-type field
(wake / command / swing) is treated as padding in the LG implementation but is functionally
significant on the Beko unit.

The Beko-specific observations not covered by the LG implementation:
- Separate wake (`000`) vs command (`001`) frame types with checksum difference of 8
- Fixed-temperature encoding for DEHUM and FAN ONLY modes
- Sessiz (outdoor silent) fixed frames `0x88C0A6C` / `0x88C0A7D`
- Economi mode (frame class identified, payload unknown)

---

Previous search results (before identifying LG origin):

- **IRremoteESP8266**: only Beko BINR 070/071 — Coolix-compatible 24-bit frame (leader 9150/4150 µs). Different model.
- **Flipper-IRDB**: only Beko BRVPF120 — different timings (4455/4310 µs leader).
- **SmartIR**: only BEVCA 120 in Broadlink format, different model.
- **GitHub code search**: hex values `0x880069F` / `0x88C0051` returned zero matches at time of search.

---

## Contribution Targets

Since the protocol is already in IRremoteESP8266 as LG, the work is simpler:

1. **IRremoteESP8266** — add `kLgAcModelArcelik` model variant with Beko-specific quirks
   (wake/cmd frame types, sessiz frames, fixed temp for DEHUM/FAN)
2. **Flipper-IRDB** `.ir` file — raw timings under Arcelik/Beko brand, straightforward
3. **SmartIR** JSON (`codes/climate/`) — map Beko model to existing LG handler
4. **ESPHome** — already works via `lg` platform if wake/cmd distinction is not needed

---

## References

- [mildsunrise — Tuya IR / FastLZ format](https://gist.github.com/mildsunrise/1d576669b63a260d2cff35fda63ec0b5)
- [zigbee2mqtt#29701 — UFO-R11 silent truncation bug](https://github.com/Koenkk/zigbee2mqtt/issues/29701)
- [zigbee2mqtt#26477 — IR codes >255 chars fail](https://github.com/Koenkk/zigbee2mqtt/issues/26477)
- [pasthev/irtuya](https://github.com/pasthev/irtuya) — Tuya IR decode/encode tool
- [irtuya web app](https://irtuya.streamlit.app/)
- [SmartIR + UFO-R11 integration guide](https://community.home-assistant.io/t/guide-how-to-use-the-zs06-or-ufo-r11-zigbee-ir-controllers-with-smartir/939301)
- [Kelon/Electra/Tadiran AC — Jinja2 IR generation example](https://community.home-assistant.io/t/control-kelon-electra-tadiran-ac-with-tuya-ir-blaster-fully-reverse-engineered-kelon168-protocol/1012947)
- [IRremoteESP8266 ir_LG.h — LG A/C protocol (identical to Beko)](https://github.com/crankyoldgit/IRremoteESP8266/blob/master/src/ir_LG.h)
- [IRremoteESP8266 — adding new AC protocol guide](https://github.com/crankyoldgit/IRremoteESP8266/wiki/Adding-support-for-a-new-AC-protocol)
- [Zigbee2MQTT UFO-R11 device page](https://www.zigbee2mqtt.io/devices/UFO-R11.html)
